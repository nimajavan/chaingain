...
import { formatUsdt, loadApi, LOTTERY_CONTRACT_PENDING, relativeTime, shortAddress, TICKET_PRICE, type IndexedActivity, type IndexedDraw } from "./data";

export default function Home() {
  const { account, openBuy, isLive, network } = useLotto();
  const [draw, setDraw] = useState<IndexedDraw | null>(null);
  const [activity, setActivity] = useState<IndexedActivity[]>([]);
  const prize = BigInt(draw?.pool_atomic ?? "0");
  const [countdown, setCountdown] = useState({ days: 0, hours: 0, minutes: 0, seconds: 0 });
  const deadlineRef = useRef(0);

  useEffect(() => {
    void Promise.all([loadApi<IndexedDraw>("/api/current"), loadApi<IndexedActivity[]>("/api/activity")]).then(([current, feed]) => {
      setDraw(current); setActivity(feed ?? []);
    });
  }, []);

  useEffect(() => {
    deadlineRef.current = (draw?.closes_at ?? 0) * 1000;
      ...
      <div className="mx-auto max-w-7xl px-4 pb-8 pt-10 sm:px-6 sm:pt-14 lg:px-8 lg:pt-16">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div className={`flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm font-medium ${isLive ? "border-emerald-400/15 bg-emerald-400/[.06] text-emerald-300" : "border-amber-400/15 bg-amber-400/[.06] text-amber-300"}`}>
            <span className={`h-2 w-2 rounded-full ${isLive ? "live-dot bg-emerald-400" : "bg-amber-400"}`} /> {isLive ? `TRON ${network} contract verified` : "Production launch gate"}
          </div>
          <div className="flex items-center gap-2 rounded-full border border-emerald-300/15 bg-emerald-300/[.05] px-3 py-1.5 text-xs font-medium text-emerald-200/80">
            <CircleDollarSign className="h-3.5 w-3.5" /> USDT only · TRON {network}
          </div>
        </div>
        ...
        <section className="grid items-stretch gap-5 lg:grid-cols-[1.35fr_.65fr]">
          <div className="glass relative overflow-hidden rounded-[26px] p-5 sm:p-8 lg:p-10">
            ...
            <div className="relative">
              <p className="text-sm font-medium uppercase tracking-[.2em] text-zinc-500">{draw ? `Draw #${draw.draw_id} pool` : "No confirmed active draw"}</p>
              ...
              <p className="mono mt-3 text-sm text-zinc-500">{formatUsdt(prize)} USDT · confirmed indexer data</p>
              ...
              <div className="mt-8 grid gap-3 border-t border-white/[.07] pt-6 sm:grid-cols-3">
                <StatChip icon={<Ticket />} label="Tickets sold" value={(draw?.total_tickets ?? 0).toLocaleString("en-US")} />
                <StatChip icon={<Users />} label="Unique players" value={(draw?.unique_players ?? 0).toLocaleString("en-US")} />
                <StatChip icon={<Sparkles />} label="Your odds" value={account && draw?.total_tickets ? `At least 1 in ${draw.total_tickets}` : "Connect to see"} />
              </div>
            </div>
          </div>
          ...
          <aside className="glass flex min-h-[470px] flex-col rounded-[26px] p-5 sm:p-6" aria-labelledby="activity-heading">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-zinc-500">Confirmed TRON event stream</p>
              </div>
              <RefreshCw className="h-4 w-4 text-zinc-600" />
            </div>
            <div className="mt-5 flex-1 divide-y divide-white/[.06]" aria-live="polite">
              {activity.map((item, index) => (
                <div key={`${item.transaction_id}-${index}`} className="feed-in flex items-center gap-3 py-4" style={{ animationDelay: `${index * 70}ms` }}>
                  ...
                  <div className="min-w-0 flex-1"><p className="mono truncate text-sm text-zinc-300">{shortAddress(item.buyer)}</p><p className="mt-0.5 text-xs text-zinc-600">bought {item.quantity} ticket{item.quantity > 1 ? "s" : ""}</p></div>
                  <span className="text-xs text-zinc-600">{relativeTime(item.block_timestamp)}</span>
                </div>
              ))}
              {!activity.length && <p className="py-16 text-center text-sm text-zinc-600">No confirmed purchases indexed.</p>}
            </div>
        ...
        <section className="mt-5 grid gap-5 lg:grid-cols-[1fr_.48fr]">
          <div className="glass rounded-[22px] p-5 sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div><p className="text-sm font-medium text-zinc-200">{draw?.unique_players ?? 0} of 500 minimum players</p><p className="mt-1 text-sm text-zinc-500">{(draw?.unique_players ?? 0) >= 500 ? "Minimum participation threshold passed" : "Refunds activate if the threshold is missed"}</p></div>
              <span className="display-font text-2xl font-semibold text-white">{Math.min(100, ((draw?.unique_players ?? 0) / 500) * 100).toFixed(1)}%</span>
            </div>
            <Progress value={Math.min(100, ((draw?.unique_players ?? 0) / 500) * 100)} className="mt-5 h-3 bg-white/[.06] [&_[data-slot=progress-indicator]]:bg-gradient-to-r [&_[data-slot=progress-indicator]]:from-violet-600 [&_[data-slot=progress-indicator]]:to-fuchsia-500" />
          ...
          <div className="glass rounded-[22px] p-5 sm:p-6">
            ...
            <p className="display-font gold-text mt-2 text-3xl font-semibold">${formatUsdt(prize * 7n / 10n)}</p>
        ...
        <section className="glass relative overflow-hidden rounded-[26px] p-6 sm:p-9">
          ...
          <div className="relative flex flex-col gap-7 md:flex-row md:items-center md:justify-between">
            ...
            <Link href="/history" className="focus-ring flex shrink-0 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[.05] px-5 py-3 text-sm font-semibold text-white transition hover:border-violet-400/30 hover:bg-white/[.08]">Inspect finalized proofs <ExternalLink className="h-4 w-4" /></Link>
          </div>
        </section>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-xs text-zinc-600">
          ...
          <span className="flex items-center gap-2"><Blocks className="h-4 w-4" /> TRON {network}</span>
